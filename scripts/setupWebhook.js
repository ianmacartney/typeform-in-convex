...
async function setupTypeformWebhook() {
    commander
        .usage('[OPTIONS]...')
        .option('--form-id <value>', 'Typeform Form ID (find in the url of your typeform)')
        .option('--deployment-url <value>', 'Convex deployment URL ((find it in .env for prod or .env.local for dev)')
        .parse(process.argv);
    ...
    if (!deploymentUrl) {
        console.log("Provide your Convex deployment URL (find it in .env for prod or .env.local for dev)");
        return;
    }
    ...
    const webhookResponse = await fetch(`https://api.typeform.com/forms/${formId}/webhooks/${tag}`, {
        method: "PUT",
        headers: new Headers({
    ...
    if (!webhookResponse.ok) {
        ...
        console.log('Go to your convex dashboard and add the following as Environment Variable:')
