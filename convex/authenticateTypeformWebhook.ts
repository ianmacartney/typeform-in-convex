...
export default internalAction(({runMutation}, {requestBodyString, signature}: ValidationArgs) => {
  const typeformSecretToken = process.env.TYPEFORM_SECRET_TOKEN;
  if (!typeformSecretToken) {
    console.error(`Must configure TYPEFORM_SECRET_TOKEN as a Convex environment variable`)
    return false;
  }
